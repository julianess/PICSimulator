...
public class SingleLayoutController {
	...
	public void onClickOpenFile() {
		...
		if (ersterStart) {
			...
			table_checkbox.setCellValueFactory(new PropertyValueFactory<ValueClass, String>(
							"text_checkbox"));
			...
			table_pcl.setCellValueFactory(new PropertyValueFactory<ValueClass, String>(
							"text_pcl"));
			table_pcl.setEditable(false);
			table_pcl.setSortable(false);
			table_pcl.setMinWidth(40);
			table_pcl.setMaxWidth(40);
			table_code.setCellValueFactory(new PropertyValueFactory<ValueClass, String>(
							"text_code"));
			table_code.setEditable(false);
			table_code.setSortable(false);
			table_code.setMinWidth(40);
			table_code.setMaxWidth(40);
			table_zusatz.setCellValueFactory(new PropertyValueFactory<ValueClass, String>(
							"text_zusatz"));
	...
	public short getLine() {
		...
		// Programcounter.pc String formatieren, sodass er immer 4-Stellig Hex
		// angezeigt wird
			...
			if (table_pcl.getCellData(i).equals(pclString)) {
				...
				Platform.runLater(new Runnable() {
					...
					public void run() {
						updateUiTable(row);
						// Breakpoint Abfrage
						if (data.get(row).getText_checkbox().selectedProperty()
								.get()) {
							pause = true;
	...
	public void erzeugeSpeicher() {
		...
		table_zahl
				.setStyle("-fx-background-color:#E2E2E2; -fx-border-color:#EDEDED; -fx-border-style: solid; -fx-border-width:0.5px");
				...
				public void run() {
					...
					for (int i = 0; i < table.getItems().size(); i++) {
						if (!table_pcl.getCellData(i).equals("")) {
							maximalerPC = (short) Integer.parseInt(
									table_pcl.getCellData(i), 16);
						}
					}
					...
					for (int i = 0; i <= maximalerPC; i = Programcounter.pc) {
						...
						// Alter Wert der Register erhalten und ggf Interrupt
						// Bits setzen
						...
						short test = getLine();
						System.out.println("PC: " + Programcounter.pc
								+ "  Code: " + test + "\n");
						...
						// Alle Register neu aus dem Speicher in ihre Arrays
						// laden
						...
						// Neuer Wert der Register erhalten und ggf Interrupt
						// Bits setzen
	...
	public void stopRunningThread() {
		if (t != null) {
			...
			// WDT wieder ein- oder ausschaltbar, wenn das Programm angehalten
			// wurde
	...
	public void felderAktualisieren() {
		...
		// BefehlDecoder.speicherZellen[RegisterAdressen.ADR_PCL_0] =
		// BefehlDecoder.speicherZellen[RegisterAdressen.ADR_PCL_1] = (short)
		// Programcounter.pcl;

		// Label wRegister aktualisieren
		label_wRegister.setText("0x"
				+ Integer.toHexString(BefehlDecoder.wRegister).toUpperCase());
		...
		label_pclath.setText("0x"
				+ Integer.toHexString(BefehlDecoder.speicherZellen[10])
						.toUpperCase());
		...
		label_fsr
				.setText("0x"
						+ Integer
								.toHexString(
										BefehlDecoder.speicherZellen[RegisterAdressen.ADR_FSR_0])
								.toUpperCase());
		// Label Programmcounter aktualisieren
		label_programcounter.setText("0x"
				+ Integer.toHexString(Programcounter.pc).toUpperCase());
		...
		label_status
				.setText("0x"
						+ Integer
								.toHexString(
										BefehlDecoder.speicherZellen[RegisterAdressen.ADR_STATUSREGISTER_0])
								.toUpperCase());
		// Label Taktzyklen aktualisieren
		label_taktzyklen.setText("0x"
				+ Integer.toHexString(taktzyklen).toUpperCase());
	...
	public void aktualisiereSpeicherView() {
		...
		for (int i = 0; i <= 31; i++) {
			...
			for (int a = 0; a < 8; a++) {
				speicherByte[a] = Integer.toHexString(
						BefehlDecoder.speicherZellen[(8 * i) + a])
						.toUpperCase();
			...
			data_speicher.add(new ValueClassSpeicher(zeileNummer,
					speicherByte[0], speicherByte[1], speicherByte[2],
					speicherByte[3], speicherByte[4], speicherByte[5],
					speicherByte[6], speicherByte[7]));
		}
		PortA.setPortA();
		PortB.setPortB();
	}
	...
	public void erzeugeFrequenzChoice() {
		choice_quarzfrequenz
				.setItems(FXCollections.observableArrayList("32.76800 kHz",
						"500.0000 kHz", "1.000000 MHz", "2.000000 MHz",
						"2.457600 MHz", "3.000000 MHz", "3.276800 MHz",
						"3.680000 MHz", "3.686411 MHz", "4.000000 MHz",
						"4.096000 MHz", "4.194304 MHz", "4.433619 MHz",
						"4.915200 MHz", "5.000000 MHz", "6.000000 MHz",
						"6.144000 MHz", "6.250000 MHz", "6.553600 MHz",
						"8.000000 MHz", "10.00000 MHz", "12.00000 MHz",
						"16.00000 MHz", "20.00000 MHz", "24.00000 MHz",
						"32.00000 MHz", "40.00000 MHz", "80.00000 MHz"));
	...
	public void setComPorts() {
		...
		ObservableList<String> availablePorts = FXCollections
				.observableArrayList(comPorts);
	...
	private void getQuarzFrequenz() {
		Laufzeit.einheit = ((String) choice_quarzfrequenz.getValue())
				.substring(9, 12);
		Laufzeit.quarzfrequenz = Double
				.parseDouble(((String) (choice_quarzfrequenz.getValue()))
						.substring(0, 8));
	}
	...
	private void connectCOM() {
		comport = new SeriellerPort(); // Objekt der Klasse SeriellerPort
										// erzeugen
	...
	private void comportCommunication() {
		...
		BefehlDecoder.speicherZellen[RegisterAdressen.ADR_PORTA] = (short) comport
				.getInputPortA();
		BefehlDecoder.speicherZellen[RegisterAdressen.ADR_PORTB] = (short) comport
				.getInputPortB();
